...
export default function DeviceServiceRequestScreenController(): React.JSX.Element {
  ...
  const filteredRequests = useMemo(() => {
    ...
    return list;
  }, [requests, historyStatusFilter, historySearchTerm]);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <CreatableCustomSelectSharedComponent
            ...
            searchPlaceholder="Search locations or type custom address..."
          />
        </div>
      ...
      {/* 2. Full-Width Past Requests History Table */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-zinc-800/80">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex items-center min-w-[240px]">
              <Search className="w-3.5 h-3.5 text-slate-400 dark:text-zinc-500 absolute left-3" />
              <input
                type="text"
                value={historySearchTerm}
                onChange={(e) => setHistorySearchTerm(e.target.value)}
                placeholder="Search ticket #, device, employee..."
                className="w-full h-8 pl-8 pr-3 text-xs bg-slate-50 dark:bg-zinc-800/60 border border-slate-200/80 dark:border-zinc-700/60 rounded-xl text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-500 focus:outline-none focus:ring-1 focus:ring-[#0C2086]/50 focus:border-[#0C2086]"
              />
            </div>

            <div className="flex items-center gap-1 p-0.5 rounded-lg bg-slate-100 dark:bg-zinc-800/80 border border-slate-200/60 dark:border-zinc-700/60 text-[11px] font-medium">
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/60">
                {filteredRequests.map((req) => (
                  ...
                  >
                    <td className="py-3.5 px-4 font-mono font-bold text-[#0C2086] dark:text-blue-400 whitespace-nowrap">
                      {req.requestNumber}
                    </td>
                    ...
                    <td className="py-3.5 px-4 whitespace-nowrap text-slate-500 dark:text-zinc-400 text-[11px]">
