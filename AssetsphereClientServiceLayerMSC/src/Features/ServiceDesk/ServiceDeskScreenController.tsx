...
export default function ServiceDeskScreenController({
  ...
  const setViewMode = (mode: 'grid' | 'list') => {
    ...
    UserPreferencesUtility.current.setServiceDeskViewMode(mode);
  };

  const setGridColumns = (cols: 2 | 3) => {
    setGridColumnsState(cols);
    UserPreferencesUtility.current.setServiceDeskGridColumns(cols);
  };

  const setIsSingleLineMode = (val: boolean) => {
    setIsSingleLineModeState(val);
    UserPreferencesUtility.current.setServiceDeskSingleLine(val);
  };
  ...
  const filteredTickets = tickets.filter(
    (t) =>
      ...
      (t.assignedEngineer || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      (t.priority || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      (t.status || '').toLowerCase().includes(searchQuery.toLowerCase())
  );

  const criticalOrHighCount = filteredTickets.filter(
  ...
  return (
    <div className="space-y-6">
      ...
      <CardSharedComponent className="p-3">
        ...
        {/* Priority Level Color Legend Indicator (Inside Same Toolbar Card) */}
        <div className="pt-3.5 mt-4 border-t border-slate-100 dark:border-zinc-800/80 flex flex-wrap items-center gap-5 text-xs font-mono">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-700 dark:text-zinc-300 font-medium">Critical Priority</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3.5 h-1.5 rounded-full bg-gradient-to-r from-amber-500 via-orange-400 to-amber-500 shadow-xs" />
            <span className="text-slate-700 dark:text-zinc-300 font-medium">High Priority</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3.5 h-1.5 rounded-full bg-gradient-to-r from-sky-500 via-blue-400 to-sky-500 shadow-xs" />
            <span className="text-slate-700 dark:text-zinc-300 font-medium">Medium / Low Priority</span>
          </div>
        </div>
      </CardSharedComponent>
      ...
      {filteredTickets.length === 0 && (
        <CardSharedComponent className="p-12 text-center space-y-2">
          ...
          <p className="text-xs text-slate-500 dark:text-zinc-400">
            No repair tickets matched your search query "{searchQuery}".
          </p>
        </CardSharedComponent>
      )}
            ...
            <table className={`w-full text-left text-xs ${isSingleLineMode ? 'min-w-[900px] whitespace-nowrap' : ''}`}>
              <thead>
                <tr className="border-b border-slate-200 dark:border-zinc-800 bg-slate-100/50 dark:bg-zinc-900/40 text-slate-400 dark:text-zinc-500 font-mono">
                  ...
                  <th className="py-3.5 px-5">Problem Summary</th>
                  <th className="py-3.5 px-5">Target Hardware Asset</th>
                  <th className="py-3.5 px-5">Assigned Engineer</th>
                  <th className="py-3.5 px-5">Priority Level</th>
                  <th className="py-3.5 px-5 text-right">Repair Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/60">
                {filteredTickets.map((t) => (
                  ...
                  >
                    <td className="py-4 px-5 font-mono font-bold text-indigo-500">
                      {t.ticketNumber}
                    </td>
                    <td className="py-4 px-5 font-serif-headline font-bold text-slate-900 dark:text-white text-sm">
                      {t.problemDescription}
                    </td>
                    <td className="py-4 px-5 font-mono text-slate-700 dark:text-zinc-300">
                      {t.assetName}
                    </td>
                    ...
                    <td
                      className={`py-4 px-5 font-mono font-bold ${
