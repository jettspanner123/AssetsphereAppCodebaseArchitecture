...
class BackendEnvironmentSwitcher:
    ...
    def __init__(self) -> None:
        ...
        self.ClientDirectory: Path = self.RootDirectory / "AssetsphereClientServiceLayerMSC"
        ...
        self.Targets: Dict[str, EnvironmentTargetClass] = {
            self.TARGET_LOCAL: EnvironmentTargetClass(
                ...
                Description="Local ASP.NET Core 10 daemon running on development machine",
                BadgeColor=TerminalThemeClass.ACCENT_CYAN,
            ),
            self.TARGET_LIVE: EnvironmentTargetClass(
                Key="live",
                Name="Render Cloud Production API",
                BaseUrl="https://assetsphereappcodebasearchitecture.onrender.com",
                Description="Live containerized Web API hosted on Render Cloud Infrastructure",
    ...
    def ProbeServerHealth(self, BaseUrl: str) -> Tuple[bool, long, str]:
        health_url = f"{BaseUrl.rstrip('/')}/Api/V1/HealthCheck"
        start_time = time.time()
        try:
            req = urllib.request.Request(
                health_url,
                headers={"User-Agent": "AssetSphere-CLI-Environment-Probe"},
            )
            with urllib.request.urlopen(req, timeout=4.0) as response:
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        overall_status = parsed.get("data", {}).get("overallStatus", "Healthy")
    ...
    def RenderStatusCard(
        ...
        health_plain_len = len(health_plain) + 2  # including icon
    ...
    def SwitchTo(self, TargetKey: str) -> None:
        ...
        self.RenderBanner(
            width,
            "ASSETSPHERE BACKEND TARGET SWITCHER",
    ...
    def ShowStatus(self) -> None:
        ...
        self.RenderBanner(
            width,
            "ASSETSPHERE BACKEND GATEWAY STATUS",
        ...
        print(f"   • Switch to Live Render Cloud:  {T.BOLD}{T.ACCENT_EMERALD}bun run backend:live{T.RESET}")
        print(f"   • Switch to Localhost Daemon:   {T.BOLD}{T.ACCENT_CYAN}bun run backend:local{T.RESET}")
        print()
